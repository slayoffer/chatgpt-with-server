...
export function Layout() {
  ...
  useEffect(() => {
    // Find the highest chat number when the component mounts
    const findHighestChatNumber = async () => {
      const chats = await db.chats.toArray();
      const highestNumber = chats.reduce((max, chat) => {
        const match = chat.description.match(/^Chat (\d+)$/);
        if (match) {
      ...
      setNextChatNumber(highestNumber + 1);
    };

    findHighestChatNumber();
  }, []);
  ...
  const createNewChat = async () => {
    ...
    setNextChatNumber((prevNumber) => prevNumber + 1);
            ...
            <Box sx={{ padding: 4 }}>
              {tab === "Chats" && (
                <Button
                  ...
                  // onClick={() => {
                  //   const randomName = `Chat ${Math.floor(
                  //     Math.random() * 10000
                  //   )}`;
                  //   const id = nanoid();
                  //   db.chats.add({
